...
export default function SubmissionUploadPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const resA = await fetch(`http://127.0.0.1:8000/api/assignments/${assignmentId}/`);
        if (!resA.ok) throw new Error("Failed to load assignment");
        const dataA = await resA.json();
        setAssignment(dataA);

        // Check for deadline extension for this student
        const resExt = await fetch(`http://127.0.0.1:8000/api/assignments/${assignmentId}/extensions/`);
        let extendedDeadline = null;
        if (resExt.ok) {
          const extensions = await resExt.json();
          // Check for personal extension
          const personalExt = extensions.find(ext => ext.user && ext.user.id === loggedInUser.id);
          if (personalExt) {
            extendedDeadline = new Date(personalExt.extended_submission_deadline);
            setHasExtension(true);
          } else {
            // Check for class-wide extension
            const classExt = extensions.find(ext => ext.user === null);
            if (classExt) {
              extendedDeadline = new Date(classExt.extended_submission_deadline);
              setHasExtension(true);
            }
          }
        }

        const originalDeadline = new Date(dataA.submission_deadline);
        const deadline = extendedDeadline || originalDeadline;
        setEffectiveDeadline(deadline);
        setIsPastDeadline(deadline < new Date());

        const resS = await fetch(
          `http://127.0.0.1:8000/api/assignments/${assignmentId}/submissions/?student=${loggedInUser.id}&current=true`
        );
        ...
        if (subs.length > 0) {
          ...
          const resF = await fetch(`http://127.0.0.1:8000/api/addfile/?submission=${sub.id}`);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!existingSubmission) {
        ...
        const resCreate = await fetch("http://127.0.0.1:8000/api/submit/", {
          method: "POST",
      ...
      if (existingFile) {
        ...
        const resUpdate = await fetch(`http://127.0.0.1:8000/api/addfile/${existingFile.id}/`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        ...
        const resCreateFile = await fetch("http://127.0.0.1:8000/api/addfile/", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payloadFile),
        });
  ...
  return (
    <div style={{ padding: "1rem", fontFamily: "sans-serif" }}>
      ...
      {hasExtension && effectiveDeadline && (
        <p style={{ color: "green", fontWeight: "bold" }}>
          <strong>Extended Submission Deadline:</strong>{" "}
          {effectiveDeadline.toLocaleString()}
        </p>
      )}
