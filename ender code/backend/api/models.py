...
class Assignment(models.Model):
    ...
    def get_effective_deadline(self, user=None):
        """Get the effective submission deadline for a user."""
        from .utils import get_effective_deadline
        return get_effective_deadline(self, user)

    def is_past_deadline(self, user=None):
        """Check if the effective deadline has passed for a user."""
        from .utils import is_past_deadline
        return is_past_deadline(self, user)

    def has_active_extension(self, user):
        """Check if a user has an active deadline extension."""
        from .utils import has_active_extension
        return has_active_extension(self, user)
...
class AssignmentDeadlineExtension(models.Model):
    """Model to store extended submission deadlines.
    If `user` is null the extension is considered class-wide.
    """
    assignment = models.ForeignKey(Assignment, on_delete=models.CASCADE, related_name='extensions')
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True)
    # This field represents an extended submission deadline (not commenting deadline)
    ...
    def __str__(self):
        target = 'ALL' if self.user is None else f'user {self.user_id}'
        return f"Submission-extension for assignment {self.assignment_id} -> {target} until {self.extended_submission_deadline}"
